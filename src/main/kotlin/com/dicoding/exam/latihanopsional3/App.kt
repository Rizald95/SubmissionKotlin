...
fun manipulateString(str: String, int: Int): String {
    ...
    //memisahkan bagian angka dan bukan angka
    str.forEach {
        if (it.isDigit()) {
            digits += it
        } else {
    ...
    //mengubah bagian angka menjadi bilangan bulat
    val nonStr = digits.toIntOrNull() ?: 0

    //memanipulasi string sesuai dengan nilai nonStr
    return if (nonStr !=0) {
        val result = nonStr * int
        nonDigits + result
    } else {
        str + int
    }
